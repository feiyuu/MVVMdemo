...
package com.feiyu.mvvmdemo.ui.callback;
...
abstract class ELiveData<T> {
     ...
     */
    public ELiveData(Event<T> value) {
        mData = value;
    ...
    private abstract class ObserverWrapper {
        ...
        void activeStateChanged(boolean newActive) {
            ...
            // owner
            mActive = newActive;
            boolean wasInactive = ELiveData.this.mActiveCount == 0;
            ELiveData.this.mActiveCount += mActive ? 1 : -1;
            if (wasInactive && mActive) {
                onActive();
            }
            if (ELiveData.this.mActiveCount == 0 && !mActive) {
                onInactive();
            }
            if (mActive) {
